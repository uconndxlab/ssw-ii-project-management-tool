<div class="mb-3">
    ...
    <input type="text"
           ...
           id="name"
           name="name"
           value="{{ old('name', $agreementLoggingField->name ?? '') }}"
           required>
    @error('name')
...
<div class="mb-3">
    ...
    <select class="form-select @error('field_type') is-invalid @enderror" id="field_type" name="field_type" required>
        ...
        @foreach($fieldTypes as $key => $label)
            <option value="{{ $key }}" {{ old('field_type', $agreementLoggingField->field_type ?? '') === $key ? 'selected' : '' }}>{{ $label }}</option>
        @endforeach
    </select>
    @error('field_type')
        <div class="invalid-feedback">{{ $message }}</div>
...
<div class="mb-3" id="options-field-wrapper">
    ...
    <textarea class="form-control @error('options_json') is-invalid @enderror"
              ...
              name="options_json"
              rows="4"
              placeholder='["Option 1", "Option 2"]'>{{ old('options_json', isset($agreementLoggingField) && $agreementLoggingField->field_type === 'select' && $agreementLoggingField->options_json ? json_encode($agreementLoggingField->options_json) : '') }}</textarea>
    @error('options_json')
        <div class="invalid-feedback">{{ $message }}</div>
    @enderror
</div>
...
<div class="mb-3">
    ...
    <textarea class="form-control @error('help_text') is-invalid @enderror" id="help_text" name="help_text" rows="2">{{ old('help_text', $agreementLoggingField->help_text ?? '') }}</textarea>
    @error('help_text')
        <div class="invalid-feedback">{{ $message }}</div>
    @enderror
</div>
